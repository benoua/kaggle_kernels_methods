...
from svm_project_utils import plot_dataset, datasets, plot_classif
...
def compute_w(X, idx_support, mu_support):
	'''
	Compute w 
	'''		
	w = (Y[idx_support] * mu_support)[:, None] * X[idx_support]
	w = w.sum(axis=0)
	return w
...
def compute_b(X, Y, w):
	...
	'''
	n = X.shape[0]
	A_minus = [np.dot(w.T, X[i]) for i in range(n) if Y[i] == -1]
	A_plus = [np.dot(w.T, X[i]) for i in range(n) if Y[i] == 1]

	b = - (max(A_minus) + min(A_plus)) /2
	return b
...
if __name__ == "__main__":
	...
	n = 1000
	C = 1
	X, Y = datasets(name='gaussian', n_points=n, sigma=1)
	# X_ones = np.concatenate((X, np.ones((len(X), 1))), axis=1) # adding intercept to X
	# K_ones = kernel(X_ones,X_ones)
	# model = quadra_pb(K=K_ones , Y=Y, C=C)
	...
	# idx_support =  np.where(np.abs(x_star)>1e-5)
	# mu_support = x_star[np.abs(x_star) > 1e-5]
	# w = compute_w(X_ones, idx_support, mu_support)
	# w, b = w[:-1], w[-1]
	...
	print("SVM.SVC intercept : %.2f " %(clf.intercept_))
	...
	idx_support = np.where(np.abs(alpha)>1e-5)
	alpha_support = alpha[idx_support]

	w = compute_w(X, idx_support, alpha_support)
	b = compute_b(X,Y,w)
	print(" Intercept  : %.2f" %(b))

	plot_classif(X, Y, alpha_support, idx_support, b, kernel=kernel)
	# plot_classif(X, Y, clf.dual_coef_[0], clf.support_, clf.intercept_, kernel=kernel)
